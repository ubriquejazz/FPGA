...
*     1.02     06/06/00  rpc     Modified for Hammerhead, as well as 21065L   *
*                                                                             *
\*****************************************************************************/
...
int *get_ms_base(int msnum)
{
	...
	dms = (int *) 0x800000 + (msnum * (0x2000 << msize));
#else
    if( msnum == 0 )
        dms = (int *)0x00020000;
    else
        dms = (int *)(0x01000000 * msnum);
#endif
    return(dms);
}
...
void init_uart1( void )
{
	...
	// Enable UART1 to interrupt the HH_PCI	
	switch(mpid)
	{
		case 1: // HH1
		   *(ms2_ptr + HH1_IRQ0) = B_UART0;
		break;
	
		case 2: // HH2
		   *(ms2_ptr + HH2_IRQ0) = B_UART0;
		break;
	
		case 3: // HH3
		   *(ms2_ptr + HH3_IRQ0) = B_UART0;
		break;
	
		case 4: // HH4
		   *(ms2_ptr + HH4_IRQ0) = B_UART0;
		break;
	}
...
#else
	// Set register 1 so that IRQ1=Uart1
    if( ((GetIOP( SYSTAT ) >> 8) & 0x07) == 1 )
    {    
        // If mpid == 1, set interrupts so this DSP only gets Uart1 interrupts.
        CTL_REG1 = IRQ1_DSPA_SEL_UART1 | CTL_REG1;
    }
    else
    {   
        // If mpid == 2, must get both interrupts to get Uart1's. 
        CTL_REG1 = ~IRQ1_DSPB_SEL_UART2 & CTL_REG1;
    }
...
{    
    // Set the MS1 pointer.
    uart_ms_ptr = (get_ms_base(1) + UART_OFFSET); //RPC - pre FINv4 was MS3
...
void init_uart2( void )
{
	...
	// Enable UART2 to interrupt the HH_PCI
	switch(mpid)
	{
		case 1: // HH1
		   *(ms2_ptr + HH1_IRQ0) = B_UART1;
		break;
	
		case 2: // HH2
		   *(ms2_ptr + HH2_IRQ0) = B_UART1;
		break;
	
		case 3: // HH3
		   *(ms2_ptr + HH3_IRQ0) = B_UART1;
		break;
	
		case 4: // HH4
		   *(ms2_ptr + HH4_IRQ0) = B_UART1;
		break;
	}
	...
	asm("BIT SET IMASK IRQ0I;");
#else
    // Set register 1 so that IRQ1=Uart2
    if( ((GetIOP( SYSTAT ) >> 8) & 0x07) == 2 )
    {    
        // If mpid == 2, set interrupts so this DSP only gets Uart2 interrupts.
        CTL_REG1 = IRQ1_DSPB_SEL_UART2 | CTL_REG1;
    }
    else
    {   
        // If mpid == 1, must get both interrupts to get Uart2's. 
        CTL_REG1 = ~IRQ1_DSPA_SEL_UART1 & CTL_REG1;
    }
