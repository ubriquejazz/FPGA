...
// 
//**********************************************************
int extmem_to_intmem( int *extsrc, int *intdest, int num_to_copy, int incr, int dma_channel, int wait )
{    
    ...
    // Prepare DMA from external memory to EPB2 (via DMAC8).
	...
	DMATime = timer_on();
    // start DMA
	SetIOP( (DMAC10+(dma_channel-10)),  dmac_ext_to_int);     // Enable, Read from extmem.
...
// 
//**********************************************************
int intmem_to_extmem( int *intsrc, int *extdest, int num_to_copy, int incr, int dma_channel, int wait )
{    
    int chan_offset, DMATime;
	...
	timer_set(0x1FFFFF, 0x1FFFFF);
	DMATime = timer_on();
	SetIOP( (DMAC10+(dma_channel-10)),  dmac_int_to_ext);     // Enable, Write to extmem.
